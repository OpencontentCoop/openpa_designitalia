          ...
          <div class="u-layout-wide u-layoutCenter u-layout-r-withGutter Grid">
            ...
            {if and( $show_title, $block.name|ne('') )}
                <div class="Grid-cell u-size1of2 u-sm-size1of4 u-md-size1of4">
                  <h3 class="openpa-widget-title u-margin-top-s">{if and($valid_node.root_node, $link_top_title)}<a href={$valid_node.root_node.url_alias|ezurl()}>{/if}{$block.name|wash()}{if and($valid_node.root_node, $link_top_title)}</a>{/if}</h3>
                </div>
              {/if}
