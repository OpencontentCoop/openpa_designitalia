...
<div class="survey-view">
	...
	{if is_set($attribute.content.survey_validation.one_answer_need_login)}
		<div class="Prose Alert Alert--error">
			<h2 class="u-text-r-l">{"You need to log in in order to answer this survey"|i18n('survey')}.</h2>
		</div>
	{else}
		{if $survey.valid|eq(false())}
			<div class="Prose Alert Alert--error">
				<h2 class="u-text-r-l">{"The survey is not active"|i18n('survey')}.</h2>
			</div>
    	{else}
			...
			{if or(is_set( $survey_validation.one_answer ), and(is_set($survey_validation.one_answer_count), $survey_validation.one_answer_count|gt(0)))}
				<div class="Prose Alert Alert--error">
					<h2 class="u-text-r-l">{"The survey does already have an answer from you"|i18n('survey')}.</h2>
				</div>
			{else}
				...
				{if $module_param_value|ne('content/edit')}
					<form class="Form Form--spaced" enctype="multipart/form-data" method="post" action={$node.url_alias|ezurl()} onsubmit="return checkSubmit();">
				{/if}
