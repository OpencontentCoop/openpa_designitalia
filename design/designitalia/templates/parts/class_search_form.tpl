                ...
                <div class="Form-field">
                    ...
                    <select class="Form-input reorder" name="Data[{$attribute.identifier}][]"
                        ...
                        {foreach $availableValues[$attribute.identifier] as $name => $count}
                            <option value="{$name|wash()}"
                                    {if and(is_set($Data[$attribute.identifier]),$Data[$attribute.identifier]|contains($name))}selected="selected"{/if}>{api_read($name).metadata.name['ita-IT']}
                                ({$count})
                            </option>
                        {/foreach}
                    </select>
