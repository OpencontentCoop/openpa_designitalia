...
<table id="table-{$table_id}" class='dt-column-filter Table Table--striped Table--withBorder'>
  <thead>
    <tr>
      {foreach $keys as $key}
      <th>{$key}</th>
...
$(document).ready(function() {
    ...
    $(elem).closest('.dataTables_wrapper').find('input, select').addClass("u-text-xxs").attr('placeholder', 'Cerca');
});
</script>
{/literal}
