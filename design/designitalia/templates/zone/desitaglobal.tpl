  ...
  {if and( is_set( $zones[0].blocks ), $zones[0].blocks|count() )}
    {foreach $zones[0].blocks as $block}
      ...
      and( eq( ezini( $block.type, 'ManualAddingOfItems', 'block.ini' ), 'disabled' ), ezini_hasvariable( $block.type, 'FetchClass', 'block.ini' )|not ) )}
        ...
        {else}
          <div
            class="{if and(is_set($block.custom_attributes.color_style), $block.custom_attributes.color_style|ne(''))}u-background-grey-20 u-padding-r-top u-padding-r-bottom{/if}">
            <section
              {if is_set($persistent_variable.has_container)}class="u-layout-wide u-layoutCenter u-layout-r-withGutter"{/if}>
