  ...
  <div class="Hero" style="background-image:url({$image});">
    ...
    <div class="Hero-content carousel-caption">
      ...
      {if $node|has_attribute( 'occhiello' )}
        <p class="u-padding-r-bottom u-text-r-xs u-xs-hidden">
          <a href="{$openpa.content_link.full_link}" class="u-textClean u-color-60 u-text-h4">{attribute_view_gui attribute=$node|attribute( 'occhiello' )}</a>
        </p>
      {else}
        <p class="u-padding-r-bottom u-text-r-xs u-xs-hidden">
      ...
      <p class="u-text-p">{$node|abstract()|openpa_shorten(200)}</p>
    </div>
  </div>
</div>
