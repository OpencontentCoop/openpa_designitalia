...
YAHOO.ez.MultiUploadBrowse = (function()
{
    // Private
    ...
    var onContentReady = function(o)
    {
        ...
        if ( YAHOO.ez.MultiUploadBrowse.cfg.fileType[0].extensions )
            this.setFileFilters(YAHOO.ez.MultiUploadBrowse.cfg.fileType);
    ...
    {
        if ( YAHOO.ez.MultiUploadBrowse.cfg.flashError )
            alert( YAHOO.ez.MultiUploadBrowse.cfg.flashError );
        else
            alert( "Could not load flash(or not yet loaded), this is needed for MultiUploadBrowse!" );
    };
    ...
    var onFileSelect = function(e)
    {
        ...
        this.upload(fileID, YAHOO.ez.MultiUploadBrowse.cfg.uploadURL, 'POST', YAHOO.ez.MultiUploadBrowse.cfg.uploadVars);
    ...
    {
        var width = Math.floor((YAHOO.ez.MultiUploadBrowse.cfg.progressBarWidth * e['bytesLoaded']) / e['bytesTotal']);
    ...
    {
        Dom.get('multiuploadProgressMessage').innerHTML = YAHOO.ez.MultiUploadBrowse.cfg.thumbnailCreated;
        ...
        if (this.uploadCounter < this.queue.length - 1) {
            ...
            Dom.get('multiuploadProgressMessage').innerHTML = YAHOO.ez.MultiUploadBrowse.cfg.allFilesRecived;
    ...
    var onUploadCompleteData = function(e)
    {
        ...
        if ( result.errors.length > 0 ){
            ...
            $.ez('ezjsctemplate::browse_list_row::'+result.contentobject_id+'::'+YAHOO.ez.MultiUploadBrowse.cfg.selectType+'::'+YAHOO.ez.MultiUploadBrowse.cfg.selectName, false, function(content){
                if (content.error_text.length) {
                    alert(content.error_text);
                }else{
                    $('#browse_table tbody').prepend(content.content);
                }
            });
        }
    };
    ...
    var cancelUpload = function()
    {
        ...
        Dom.get('multiuploadProgressMessage').innerHTML = YAHOO.ez.MultiUploadBrowse.cfg.uploadCanceled;
        Dom.get('uploadButton').disabled = false;
    };
    ...
    return {
        ...
        init: function() {
            Event.onDOMReady(function()
            {
                ...
                YAHOO.widget.Uploader.SWFURL = YAHOO.ez.MultiUploadBrowse.cfg.swfURL;
